...
curve_table_navic <- 
  read_csv("./data/navicular/import/navicular-curve-table.csv")

# identify the folder where your pts files are
ptsfolder_navic <- 
  "./data/navicular/import/pts"
...
save(
  my_curves_navic, 
  file = "./data/navicular/output/navicular_curve-info.R"
)
...
slid_navic <- 
  slider3d(
    subsampled_lm_navic, 
    SMvector = my_curves_navic$Sliding.LMs,
    outlines = my_curves_navic$Curves, 
    sur.path = plypath_navic, 
    meshlist = paste(
      "./data/navicular/import/ply/", 
      dimnames(subsampled_lm_navic)[[3]], 
      ".ply",sep = ""
    ), 
    ignore = NULL,
    sur.type = "ply", 
...
# source("./scripts/curve-correction-navicular.R")
...
gpa_individual_navic <- 
  gpagen(slidLMs_navicular)


# save and export gpa output
save(
  gpa_individual_navic, 
  file = "./data/navicular/output/navicular_individual-gpa-output.R"
)


# extract shape data from gpa
shape_data_individual_navic <- 
  gpa_individual_navic$coords
...
save(
  shape_data_individual_navic, 
  file = "./data/navicular/output/navicular_individual-gpa-coords.R"
)

# as csv
write.csv(
  x = two.d.array(shape_data_individual_navic),
  file = "./data/navicular/output/navicular_individual-gpa-coords.csv"
)


## extract size data (cs) from gpa
csize_individual_navic <-
  gpa_individual_navic$Csize
...
save(
  csize_individual_navic,
  file = "./data/navicular/output/navicular_individual-csize.R"
)
  
# as csv
write.csv(
  x = csize_individual_navic,
  file = "./data/navicular/output/navicular_individual-csize.csv"
)
  ...
  lapply(
    ...
    function(x) 
      ...
      mshape(
        ...
        shape_data_individual_navic[,,which(
...
save(
  shape_data_means_navic, 
  file = "./data/navicular/output/navicular_species-gpa-coords.R"
)

# as csv
write.csv(
  x = two.d.array(shape_data_means_navic),
  file = "./data/navicular/output/navicular_species-gpa-coords.csv"
)
...
csize_means_navic <- 
  as.numeric(csize_means_navic)

# check
is.numeric(csize_means_navic)


# rename cs means
names(csize_means_navic) <-
  # to match levels of species factor 
  levels(species_factor)
...
save(
  csize_means_navic, 
  file = "./data/navicular/output/navicular_species-csize.R"
)

# as csv
write.csv(
  csize_means_navic,
  file = "./data/navicular/output/navicular_species-csize.csv"
)
